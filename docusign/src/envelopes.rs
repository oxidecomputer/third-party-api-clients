...
if !ac_status.is_empty() { query_args.push(format!("ac_status={}", ac_status)); }
if !block.is_empty() { query_args.push(format!("block={}", block)); }
if !cdse_mode.is_empty() { query_args.push(format!("cdse_mode={}", cdse_mode)); }
if !continuation_token.is_empty() { query_args.push(format!("continuation_token={}", continuation_token)); }
if !count.is_empty() { query_args.push(format!("count={}", count)); }
if !custom_field.is_empty() { query_args.push(format!("custom_field={}", custom_field)); }
if !email.is_empty() { query_args.push(format!("email={}", email)); }
if !envelope_ids.is_empty() { query_args.push(format!("envelope_ids={}", envelope_ids)); }
if !exclude.is_empty() { query_args.push(format!("exclude={}", exclude)); }
...
if !from_to_status.is_empty() { query_args.push(format!("from_to_status={}", from_to_status)); }
if !include.is_empty() { query_args.push(format!("include={}", include)); }
if !include_purge_information.is_empty() { query_args.push(format!("include_purge_information={}", include_purge_information)); }
if !intersecting_folder_ids.is_empty() { query_args.push(format!("intersecting_folder_ids={}", intersecting_folder_ids)); }
if !last_queried_date.is_empty() { query_args.push(format!("last_queried_date={}", last_queried_date)); }
if !order.is_empty() { query_args.push(format!("order={}", order)); }
if !order_by.is_empty() { query_args.push(format!("order_by={}", order_by)); }
if !powerformids.is_empty() { query_args.push(format!("powerformids={}", powerformids)); }
if !query_budget.is_empty() { query_args.push(format!("query_budget={}", query_budget)); }
if !requester_date_format.is_empty() { query_args.push(format!("requester_date_format={}", requester_date_format)); }
...
if !status.is_empty() { query_args.push(format!("status={}", status)); }
if !to_date.is_empty() { query_args.push(format!("to_date={}", to_date)); }
if !transaction_ids.is_empty() { query_args.push(format!("transaction_ids={}", transaction_ids)); }
...
if !user_name.is_empty() { query_args.push(format!("user_name={}", user_name)); }
...
let mut query_args: Vec<String> = Default::default();
if !cdse_mode.is_empty() { query_args.push(format!("cdse_mode={}", cdse_mode)); }
if !change_routing_order.is_empty() { query_args.push(format!("change_routing_order={}", change_routing_order)); }
if !completed_documents_only.is_empty() { query_args.push(format!("completed_documents_only={}", completed_documents_only)); }
if !merge_roles_on_draft.is_empty() { query_args.push(format!("merge_roles_on_draft={}", merge_roles_on_draft)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
                    }
...
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !advanced_update.is_empty() { query_args.push(format!("advanced_update={}", advanced_update)); }
if !include.is_empty() { query_args.push(format!("include={}", include)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
...
let url =
format!("/v2.1/accounts/{}/envelopes/{}?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&envelope_id.to_string()),query);
...
let mut query_args: Vec<String> = Default::default();
if !advanced_update.is_empty() { query_args.push(format!("advanced_update={}", advanced_update)); }
if !resend_envelope.is_empty() { query_args.push(format!("resend_envelope={}", resend_envelope)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
                    }
...
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&envelope_id.to_string()),crate::progenitor_support::encode_path(&document_id.to_string()),crate::progenitor_support::encode_path(&page_number.to_string()),query);
...
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&envelope_id.to_string()),crate::progenitor_support::encode_path(&recipient_id.to_string()),query);
