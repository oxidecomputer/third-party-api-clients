...
pub async fn get(
&self,
account_id: &str, ac_status: &str, block: &str, cdse_mode: &str, continuation_token: &str, count: &str, custom_field: &str, email: &str, envelope_ids: &str, exclude: &str, folder_ids: &str, folder_types: &str, from_date: &str, from_to_status: &str, include: &str, include_purge_information: &str, intersecting_folder_ids: &str, last_queried_date: &str, order: &str, order_by: &str, powerformids: &str, query_budget: &str, requester_date_format: &str, search_text: &str, start_position: &str, status: &str, to_date: &str, transaction_ids: &str, user_filter: &str, user_id: &str, user_name: &str,
) -> Result<crate::types::EnvelopesInformation> {
let mut query_args: Vec<(String, String)> = Default::default();
if !ac_status.is_empty() { query_args.push(("ac_status".to_string(), ac_status.to_string())); }
if !block.is_empty() { query_args.push(("block".to_string(), block.to_string())); }
if !cdse_mode.is_empty() { query_args.push(("cdse_mode".to_string(), cdse_mode.to_string())); }
if !continuation_token.is_empty() { query_args.push(("continuation_token".to_string(), continuation_token.to_string())); }
if !count.is_empty() { query_args.push(("count".to_string(), count.to_string())); }
if !custom_field.is_empty() { query_args.push(("custom_field".to_string(), custom_field.to_string())); }
if !email.is_empty() { query_args.push(("email".to_string(), email.to_string())); }
if !envelope_ids.is_empty() { query_args.push(("envelope_ids".to_string(), envelope_ids.to_string())); }
if !exclude.is_empty() { query_args.push(("exclude".to_string(), exclude.to_string())); }
if !folder_ids.is_empty() { query_args.push(("folder_ids".to_string(), folder_ids.to_string())); }
if !folder_types.is_empty() { query_args.push(("folder_types".to_string(), folder_types.to_string())); }
if !from_date.is_empty() { query_args.push(("from_date".to_string(), from_date.to_string())); }
if !from_to_status.is_empty() { query_args.push(("from_to_status".to_string(), from_to_status.to_string())); }
if !include.is_empty() { query_args.push(("include".to_string(), include.to_string())); }
if !include_purge_information.is_empty() { query_args.push(("include_purge_information".to_string(), include_purge_information.to_string())); }
if !intersecting_folder_ids.is_empty() { query_args.push(("intersecting_folder_ids".to_string(), intersecting_folder_ids.to_string())); }
if !last_queried_date.is_empty() { query_args.push(("last_queried_date".to_string(), last_queried_date.to_string())); }
if !order.is_empty() { query_args.push(("order".to_string(), order.to_string())); }
if !order_by.is_empty() { query_args.push(("order_by".to_string(), order_by.to_string())); }
if !powerformids.is_empty() { query_args.push(("powerformids".to_string(), powerformids.to_string())); }
if !query_budget.is_empty() { query_args.push(("query_budget".to_string(), query_budget.to_string())); }
if !requester_date_format.is_empty() { query_args.push(("requester_date_format".to_string(), requester_date_format.to_string())); }
if !search_text.is_empty() { query_args.push(("search_text".to_string(), search_text.to_string())); }
if !start_position.is_empty() { query_args.push(("start_position".to_string(), start_position.to_string())); }
if !status.is_empty() { query_args.push(("status".to_string(), status.to_string())); }
if !to_date.is_empty() { query_args.push(("to_date".to_string(), to_date.to_string())); }
if !transaction_ids.is_empty() { query_args.push(("transaction_ids".to_string(), transaction_ids.to_string())); }
if !user_filter.is_empty() { query_args.push(("user_filter".to_string(), user_filter.to_string())); }
if !user_id.is_empty() { query_args.push(("user_id".to_string(), user_id.to_string())); }
if !user_name.is_empty() { query_args.push(("user_name".to_string(), user_name.to_string())); }
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/v2.1/accounts/{}/envelopes?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),query_);
...
pub async fn post(
&self,
account_id: &str, cdse_mode: &str, change_routing_order: &str, completed_documents_only: &str, merge_roles_on_draft: &str,
body: &crate::types::EnvelopeDefinition
) -> Result<crate::types::EnvelopeSummary> {
let mut query_args: Vec<(String, String)> = Default::default();
if !cdse_mode.is_empty() { query_args.push(("cdse_mode".to_string(), cdse_mode.to_string())); }
if !change_routing_order.is_empty() { query_args.push(("change_routing_order".to_string(), change_routing_order.to_string())); }
if !completed_documents_only.is_empty() { query_args.push(("completed_documents_only".to_string(), completed_documents_only.to_string())); }
if !merge_roles_on_draft.is_empty() { query_args.push(("merge_roles_on_draft".to_string(), merge_roles_on_draft.to_string())); }
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/v2.1/accounts/{}/envelopes?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),query_);
...
&self,
account_id: &str, ac_status: &str, block: &str, count: &str, email: &str, envelope_ids: &str, from_date: &str, from_to_status: &str, start_position: &str, status: &str, to_date: &str, transaction_ids: &str, user_name: &str,
...
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/v2.1/accounts/{}/envelopes/status?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),query_);
...
&self,
account_id: &str, envelope_id: &str, advanced_update: &str, include: &str,
) -> Result<crate::types::Envelope> {
let mut query_args: Vec<(String, String)> = Default::default();
if !advanced_update.is_empty() { query_args.push(("advanced_update".to_string(), advanced_update.to_string())); }
if !include.is_empty() { query_args.push(("include".to_string(), include.to_string())); }
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/v2.1/accounts/{}/envelopes/{}?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&envelope_id.to_string()),query_);
...
pub async fn put(
&self,
account_id: &str, envelope_id: &str, advanced_update: &str, resend_envelope: &str,
body: &crate::types::Envelope
) -> Result<crate::types::EnvelopeUpdateSummary> {
let mut query_args: Vec<(String, String)> = Default::default();
if !advanced_update.is_empty() { query_args.push(("advanced_update".to_string(), advanced_update.to_string())); }
if !resend_envelope.is_empty() { query_args.push(("resend_envelope".to_string(), resend_envelope.to_string())); }
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/v2.1/accounts/{}/envelopes/{}?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&envelope_id.to_string()),query_);
...
let url =
format!("/v2.1/accounts/{}/envelopes/{}/audit_events",
...
&self,
account_id: &str, document_id: &str, envelope_id: &str, count: &str, dpi: &str, max_height: &str, max_width: &str, nocache: &str, show_changes: &str, start_position: &str,
) -> Result<crate::types::PageImages> {
let mut query_args: Vec<(String, String)> = Default::default();
if !count.is_empty() { query_args.push(("count".to_string(), count.to_string())); }
if !dpi.is_empty() { query_args.push(("dpi".to_string(), dpi.to_string())); }
if !max_height.is_empty() { query_args.push(("max_height".to_string(), max_height.to_string())); }
if !max_width.is_empty() { query_args.push(("max_width".to_string(), max_width.to_string())); }
if !nocache.is_empty() { query_args.push(("nocache".to_string(), nocache.to_string())); }
if !show_changes.is_empty() { query_args.push(("show_changes".to_string(), show_changes.to_string())); }
if !start_position.is_empty() { query_args.push(("start_position".to_string(), start_position.to_string())); }
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/v2.1/accounts/{}/envelopes/{}/documents/{}/pages?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&envelope_id.to_string()),crate::progenitor_support::encode_path(&document_id.to_string()),query_);
...
&self,
account_id: &str, document_id: &str, envelope_id: &str, page_number: &str, dpi: &str, max_height: &str, max_width: &str, show_changes: &str,
) -> Result<()> {
let mut query_args: Vec<(String, String)> = Default::default();
if !dpi.is_empty() { query_args.push(("dpi".to_string(), dpi.to_string())); }
if !max_height.is_empty() { query_args.push(("max_height".to_string(), max_height.to_string())); }
...
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/v2.1/accounts/{}/envelopes/{}/documents/{}/pages/{}/page_image?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&envelope_id.to_string()),crate::progenitor_support::encode_path(&document_id.to_string()),crate::progenitor_support::encode_path(&page_number.to_string()),query_);
...
let url =
format!("/v2.1/accounts/{}/envelopes/{}/notification",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&envelope_id.to_string()),);
...
&self,
account_id: &str, envelope_id: &str, recipient_id: &str, include_chrome: &str,
) -> Result<()> {
let mut query_args: Vec<(String, String)> = Default::default();
if !include_chrome.is_empty() { query_args.push(("include_chrome".to_string(), include_chrome.to_string())); }
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/v2.1/accounts/{}/envelopes/{}/recipients/{}/initials_image?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&envelope_id.to_string()),crate::progenitor_support::encode_path(&recipient_id.to_string()),query_);
...
let url =
format!("/v2.1/accounts/{}/envelopes/{}/recipients/{}/signature",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&envelope_id.to_string()),crate::progenitor_support::encode_path(&recipient_id.to_string()),);
...
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/v2.1/accounts/{}/envelopes/{}/recipients/{}/signature_image?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&envelope_id.to_string()),crate::progenitor_support::encode_path(&recipient_id.to_string()),query_);
