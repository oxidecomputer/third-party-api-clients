...
if !exclude_distributor_brand.is_empty() { query_args.push(format!("exclude_distributor_brand={}", exclude_distributor_brand)); }
if !include_logos.is_empty() { query_args.push(format!("include_logos={}", include_logos)); }
...
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !include_external_references.is_empty() { query_args.push(format!("include_external_references={}", include_external_references)); }
if !include_logos.is_empty() { query_args.push(format!("include_logos={}", include_logos)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
...
let url =
format!("/v2.1/accounts/{}/brands/{}?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&brand_id.to_string()),query);
...
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !langcode.is_empty() { query_args.push(format!("langcode={}", langcode)); }
if !return_master.is_empty() { query_args.push(format!("return_master={}", return_master)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
                    }
...
let url =
format!("/v2.1/accounts/{}/brands/{}/resources/{}?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&brand_id.to_string()),crate::progenitor_support::encode_path(&resource_content_type.to_string()),query);
