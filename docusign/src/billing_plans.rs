...
if !include_credit_card_information.is_empty() { query_args.push(("include_credit_card_information".to_string(), include_credit_card_information.to_string())); }
if !include_metadata.is_empty() { query_args.push(("include_metadata".to_string(), include_metadata.to_string())); }
if !include_successor_plans.is_empty() { query_args.push(("include_successor_plans".to_string(), include_successor_plans.to_string())); }
...
let mut query_args: Vec<(String, String)> = Default::default();
if !preview_billing_plan.is_empty() { query_args.push(("preview_billing_plan".to_string(), preview_billing_plan.to_string())); }
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/v2.1/accounts/{}/billing_plan?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),query_);
