...
pub async fn get(
&self,
account_id: &str, count: &str, group_type: &str, include_usercount: &str, search_text: &str, start_position: &str,
) -> Result<crate::types::GroupInformation> {
let mut query_args: Vec<(String, String)> = Default::default();
if !count.is_empty() { query_args.push(("count".to_string(), count.to_string())); }
if !group_type.is_empty() { query_args.push(("group_type".to_string(), group_type.to_string())); }
if !include_usercount.is_empty() { query_args.push(("include_usercount".to_string(), include_usercount.to_string())); }
