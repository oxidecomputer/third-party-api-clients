...
let url =
format!("/v2.1/accounts/{}/permission_profiles/{}?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&permission_profile_id.to_string()),query_);
...
let mut query_args: Vec<(String, String)> = Default::default();
if !move_users_to.is_empty() { query_args.push(("move_users_to".to_string(), move_users_to.to_string())); }
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/v2.1/accounts/{}/permission_profiles/{}?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&permission_profile_id.to_string()),query_);
