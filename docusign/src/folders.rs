...
&self,
account_id: &str, folder_id: &str, from_date: &str, include_items: &str, owner_email: &str, owner_name: &str, search_text: &str, start_position: &str, status: &str, to_date: &str,
) -> Result<crate::types::FolderItemsResponse> {
let mut query_args: Vec<(String, String)> = Default::default();
if !from_date.is_empty() { query_args.push(("from_date".to_string(), from_date.to_string())); }
if !include_items.is_empty() { query_args.push(("include_items".to_string(), include_items.to_string())); }
if !owner_email.is_empty() { query_args.push(("owner_email".to_string(), owner_email.to_string())); }
if !owner_name.is_empty() { query_args.push(("owner_name".to_string(), owner_name.to_string())); }
...
let url =
format!("/v2.1/accounts/{}/folders/{}?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&folder_id.to_string()),query_);
...
let url =
format!("/v2.1/accounts/{}/folders/{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&folder_id.to_string()),);
...
&self,
account_id: &str, search_folder_id: &str, all: &str, count: &str, from_date: &str, include_recipients: &str, order: &str, order_by: &str, start_position: &str, to_date: &str,
) -> Result<crate::types::FolderItemResponse> {
let mut query_args: Vec<(String, String)> = Default::default();
if !all.is_empty() { query_args.push(("all".to_string(), all.to_string())); }
if !count.is_empty() { query_args.push(("count".to_string(), count.to_string())); }
if !from_date.is_empty() { query_args.push(("from_date".to_string(), from_date.to_string())); }
if !include_recipients.is_empty() { query_args.push(("include_recipients".to_string(), include_recipients.to_string())); }
...
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/v2.1/accounts/{}/search_folders/{}?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&search_folder_id.to_string()),query_);
