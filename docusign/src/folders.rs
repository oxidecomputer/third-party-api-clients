...
if !include_items.is_empty() { query_args.push(format!("include_items={}", include_items)); }
if !start_position.is_empty() { query_args.push(format!("start_position={}", start_position)); }
if !template.is_empty() { query_args.push(format!("template={}", template)); }
...
if !include_items.is_empty() { query_args.push(format!("include_items={}", include_items)); }
if !owner_email.is_empty() { query_args.push(format!("owner_email={}", owner_email)); }
if !owner_name.is_empty() { query_args.push(format!("owner_name={}", owner_name)); }
...
let url =
format!("/v2.1/accounts/{}/folders/{}?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&folder_id.to_string()),query);
...
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !all.is_empty() { query_args.push(format!("all={}", all)); }
if !count.is_empty() { query_args.push(format!("count={}", count)); }
if !from_date.is_empty() { query_args.push(format!("from_date={}", from_date)); }
if !include_recipients.is_empty() { query_args.push(format!("include_recipients={}", include_recipients)); }
...
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&search_folder_id.to_string()),query);
