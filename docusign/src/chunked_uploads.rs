...
let url =
format!("/v2.1/accounts/{}/chunked_uploads/{}?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&chunked_upload_id.to_string()),query);
...
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !action.is_empty() { query_args.push(format!("action={}", action)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
                    }
