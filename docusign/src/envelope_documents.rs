...
if !documents_by_userid.is_empty() { query_args.push(format!("documents_by_userid={}", documents_by_userid)); }
if !include_document_size.is_empty() { query_args.push(format!("include_document_size={}", include_document_size)); }
if !include_metadata.is_empty() { query_args.push(format!("include_metadata={}", include_metadata)); }
if !include_tabs.is_empty() { query_args.push(format!("include_tabs={}", include_tabs)); }
if !recipient_id.is_empty() { query_args.push(format!("recipient_id={}", recipient_id)); }
if !shared_user_id.is_empty() { query_args.push(format!("shared_user_id={}", shared_user_id)); }
...
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !certificate.is_empty() { query_args.push(format!("certificate={}", certificate)); }
if !documents_by_userid.is_empty() { query_args.push(format!("documents_by_userid={}", documents_by_userid)); }
if !encoding.is_empty() { query_args.push(format!("encoding={}", encoding)); }
if !encrypt.is_empty() { query_args.push(format!("encrypt={}", encrypt)); }
if !language.is_empty() { query_args.push(format!("language={}", language)); }
if !recipient_id.is_empty() { query_args.push(format!("recipient_id={}", recipient_id)); }
if !shared_user_id.is_empty() { query_args.push(format!("shared_user_id={}", shared_user_id)); }
if !show_changes.is_empty() { query_args.push(format!("show_changes={}", show_changes)); }
if !watermark.is_empty() { query_args.push(format!("watermark={}", watermark)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
                    }
