...
if !include_account_settings.is_empty() { query_args.push(format!("include_account_settings={}", include_account_settings)); }
...
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !include_charges.is_empty() { query_args.push(format!("include_charges={}", include_charges)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
                    }
...
if !envelopes_not_shared_user_status.is_empty() { query_args.push(format!("envelopes_not_shared_user_status={}", envelopes_not_shared_user_status)); }
if !folder_ids.is_empty() { query_args.push(format!("folder_ids={}", folder_ids)); }
if !item_type.is_empty() { query_args.push(format!("item_type={}", item_type)); }
if !search_text.is_empty() { query_args.push(format!("search_text={}", search_text)); }
if !shared.is_empty() { query_args.push(format!("shared={}", shared)); }
if !start_position.is_empty() { query_args.push(format!("start_position={}", start_position)); }
if !user_ids.is_empty() { query_args.push(format!("user_ids={}", user_ids)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
                    }
...
let mut query_args: Vec<String> = Default::default();
if !item_type.is_empty() { query_args.push(format!("item_type={}", item_type)); }
if !preserve_existing_shared_access.is_empty() { query_args.push(format!("preserve_existing_shared_access={}", preserve_existing_shared_access)); }
if !user_ids.is_empty() { query_args.push(format!("user_ids={}", user_ids)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
