...
if !additional_info.is_empty() { query_args.push(format!("additional_info={}", additional_info)); }
if !count.is_empty() { query_args.push(format!("count={}", count)); }
if !email.is_empty() { query_args.push(format!("email={}", email)); }
if !email_substring.is_empty() { query_args.push(format!("email_substring={}", email_substring)); }
if !group_id.is_empty() { query_args.push(format!("group_id={}", group_id)); }
if !include_usersettings_for_csv.is_empty() { query_args.push(format!("include_usersettings_for_csv={}", include_usersettings_for_csv)); }
if !login_status.is_empty() { query_args.push(format!("login_status={}", login_status)); }
if !not_group_id.is_empty() { query_args.push(format!("not_group_id={}", not_group_id)); }
...
if !user_name_substring.is_empty() { query_args.push(format!("user_name_substring={}", user_name_substring)); }
...
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !delete.is_empty() { query_args.push(format!("delete={}", delete)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
                    }
...
let url =
format!("/v2.1/accounts/{}/users/{}?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&user_id.to_string()),query);
...
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !encoding.is_empty() { query_args.push(format!("encoding={}", encoding)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
                    }
...
*   .
*/
pub async fn user_profile_image_put(
&self,
account_id: &str, user_id: &str,
) -> Result<()> {
let url =
...
*   .
*/
pub async fn user_profile_image_delete(
&self,
account_id: &str, user_id: &str,
) -> Result<()> {
let url =
