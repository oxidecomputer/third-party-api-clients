...
if !include_files.is_empty() { query_args.push(format!("include_files={}", include_files)); }
if !include_sub_folders.is_empty() { query_args.push(format!("include_sub_folders={}", include_sub_folders)); }
if !include_thumbnails.is_empty() { query_args.push(format!("include_thumbnails={}", include_thumbnails)); }
if !include_user_detail.is_empty() { query_args.push(format!("include_user_detail={}", include_user_detail)); }
if !start_position.is_empty() { query_args.push(format!("start_position={}", start_position)); }
if !workspace_user_id.is_empty() { query_args.push(format!("workspace_user_id={}", workspace_user_id)); }
...
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&workspace_id.to_string()),crate::progenitor_support::encode_path(&folder_id.to_string()),query);
...
if !pdf_version.is_empty() { query_args.push(format!("pdf_version={}", pdf_version)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
                    }
...
let url =
format!("/v2.1/accounts/{}/workspaces/{}/folders/{}/files/{}?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&workspace_id.to_string()),crate::progenitor_support::encode_path(&folder_id.to_string()),crate::progenitor_support::encode_path(&file_id.to_string()),query);
