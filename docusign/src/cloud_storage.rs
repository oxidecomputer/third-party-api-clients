...
if !cloud_storage_folder_path.is_empty() { query_args.push(("cloud_storage_folder_path".to_string(), cloud_storage_folder_path.to_string())); }
...
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&user_id.to_string()),crate::progenitor_support::encode_path(&service_id.to_string()),query_);
...
let mut query_args: Vec<(String, String)> = Default::default();
if !cloud_storage_folder_path.is_empty() { query_args.push(("cloud_storage_folder_path".to_string(), cloud_storage_folder_path.to_string())); }
if !cloud_storage_folderid_plain.is_empty() { query_args.push(("cloud_storage_folderid_plain".to_string(), cloud_storage_folderid_plain.to_string())); }
if !count.is_empty() { query_args.push(("count".to_string(), count.to_string())); }
if !order.is_empty() { query_args.push(("order".to_string(), order.to_string())); }
if !order_by.is_empty() { query_args.push(("order_by".to_string(), order_by.to_string())); }
if !search_text.is_empty() { query_args.push(("search_text".to_string(), search_text.to_string())); }
...
let url =
format!("/v2.1/accounts/{}/users/{}/cloud_storage/{}/folders/{}?{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&user_id.to_string()),crate::progenitor_support::encode_path(&service_id.to_string()),crate::progenitor_support::encode_path(&folder_id.to_string()),query_);
