...
pub async fn get(
&self,
employee_id_or_uuid: &str, include: &[String],
...
let url =
format!("/v1/employees/{}?{}",
crate::progenitor_support::encode_path(&employee_id_or_uuid.to_string()),query);
...
&self,
company_id_or_uuid: &str, terminated: bool, page: f64, per: f64, include: &[String],
...
if terminated { query_args.push(format!("terminated={}", terminated)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
                    }
...
&self,
company_id_or_uuid: &str, terminated: bool, include: &[String],
) -> Result<Vec<crate::types::Employee>> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !include.is_empty() { query_args.push(format!("include={}", include.join(" "))); }
...
let url =
format!("/v1/employees/{}/home_address",
