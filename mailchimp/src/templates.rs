...
pub async fn get(
&self,
fields: &[String], exclude_fields: &[String], count: i64, offset: i64, created_by: &str, since_date_created: &str, before_date_created: &str, type_: &str, category: &str, folder_id: &str, sort_field: crate::types::GetTemplatesSortField, sort_dir: crate::types::SortDir,
...
if !category.is_empty() { query_args.push(("category".to_string(), category.to_string())); }
...
&self,
fields: &[String], exclude_fields: &[String], template_id: &str,
) -> Result<crate::types::Templates> {
let mut query_args: Vec<(String, String)> = Default::default();
if !exclude_fields.is_empty() { query_args.push(("exclude_fields".to_string(), exclude_fields.join(" "))); }
if !fields.is_empty() { query_args.push(("fields".to_string(), fields.join(" "))); }
...
let url =
format!("/templates/{}?{}",
crate::progenitor_support::encode_path(&template_id.to_string()),query_);
...
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/templates/{}/default-content?{}",
crate::progenitor_support::encode_path(&template_id.to_string()),query_);
