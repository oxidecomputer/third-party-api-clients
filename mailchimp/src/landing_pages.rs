...
&self,
sort_dir: crate::types::SortDir, sort_field: crate::types::GetAllLandingPagesSortField, fields: &[String], exclude_fields: &[String], count: i64,
...
let mut query_args: Vec<(String, String)> = Default::default();
if use_default_list { query_args.push(("use_default_list".to_string(), use_default_list.to_string())); }
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/landing-pages?{}",
query_);
...
pub async fn get(
&self,
fields: &[String], exclude_fields: &[String], page_id: &str,
) -> Result<crate::types::LandingPage> {
let mut query_args: Vec<(String, String)> = Default::default();
if !exclude_fields.is_empty() { query_args.push(("exclude_fields".to_string(), exclude_fields.join(" "))); }
if !fields.is_empty() { query_args.push(("fields".to_string(), fields.join(" "))); }
...
let url =
format!("/landing-pages/{}?{}",
crate::progenitor_support::encode_path(&page_id.to_string()),query_);
...
let url =
format!("/landing-pages/{}/actions/publish",
crate::progenitor_support::encode_path(&page_id.to_string()),);
...
* * `page_id: &str` -- The unique id for the page.
*/
pub async fn post_actions_unpublish(
&self,
page_id: &str,
) -> Result<()> {
let url =
format!("/landing-pages/{}/actions/unpublish",
crate::progenitor_support::encode_path(&page_id.to_string()),);
