...
&self,
fields: &[String], exclude_fields: &[String], count: i64, offset: i64, type_: &str, created_by: &str, before_created_at: &str, since_created_at: &str, sort_field: crate::types::GetFileManagerFilesSortField, sort_dir: crate::types::SortDir,
...
if !created_by.is_empty() { query_args.push(("created_by".to_string(), created_by.to_string())); }
...
&self,
fields: &[String], exclude_fields: &[String], file_id: &str,
) -> Result<crate::types::Files> {
let mut query_args: Vec<(String, String)> = Default::default();
if !exclude_fields.is_empty() { query_args.push(("exclude_fields".to_string(), exclude_fields.join(" "))); }
if !fields.is_empty() { query_args.push(("fields".to_string(), fields.join(" "))); }
...
let url =
format!("/file-manager/files/{}?{}",
crate::progenitor_support::encode_path(&file_id.to_string()),query_);
...
&self,
fields: &[String], exclude_fields: &[String], count: i64, offset: i64, created_by: &str, before_created_at: &str, since_created_at: &str,
...
let url =
"/file-manager/folders".to_string();
self.client.post(&url, Some(reqwest::Body::from(serde_json::to_vec(body)?))).await
}
...
&self,
fields: &[String], exclude_fields: &[String], folder_id: &str,
) -> Result<crate::types::FileManagerFoldersGalleryFolder> {
let mut query_args: Vec<(String, String)> = Default::default();
if !exclude_fields.is_empty() { query_args.push(("exclude_fields".to_string(), exclude_fields.join(" "))); }
if !fields.is_empty() { query_args.push(("fields".to_string(), fields.join(" "))); }
...
let url =
format!("/file-manager/folders/{}?{}",
crate::progenitor_support::encode_path(&folder_id.to_string()),query_);
