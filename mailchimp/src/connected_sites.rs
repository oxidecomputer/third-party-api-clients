...
&self,
fields: &[String], exclude_fields: &[String], connected_site_id: &str,
) -> Result<crate::types::Sites> {
let mut query_args: Vec<(String, String)> = Default::default();
if !exclude_fields.is_empty() { query_args.push(("exclude_fields".to_string(), exclude_fields.join(" "))); }
if !fields.is_empty() { query_args.push(("fields".to_string(), fields.join(" "))); }
...
let url =
format!("/connected-sites/{}?{}",
crate::progenitor_support::encode_path(&connected_site_id.to_string()),query_);
...
let url =
format!("/connected-sites/{}/actions/verify-script-installation",
crate::progenitor_support::encode_path(&connected_site_id.to_string()),);
