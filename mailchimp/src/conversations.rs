...
pub async fn get(
&self,
fields: &[String], exclude_fields: &[String], count: i64, offset: i64, has_unread_messages: crate::types::IsRead, list_id: &str, campaign_id: &str,
...
if !has_unread_messages.to_string().is_empty() {  query_args.push(("has_unread_messages".to_string(), has_unread_messages.to_string())); }
...
&self,
fields: &[String], exclude_fields: &[String], conversation_id: &str,
...
let url =
format!("/conversations/{}?{}",
crate::progenitor_support::encode_path(&conversation_id.to_string()),query_);
...
&self,
fields: &[String], exclude_fields: &[String], conversation_id: &str, is_read: crate::types::IsRead, before_timestamp: Option<chrono::DateTime<chrono::Utc>>, since_timestamp: Option<chrono::DateTime<chrono::Utc>>,
) -> Result<crate::types::CollectionOfConversationMessages> {
let mut query_args: Vec<(String, String)> = Default::default();
if let Some(date) = before_timestamp { query_args.push(("before_timestamp".to_string(), date.to_rfc3339())); }
if !exclude_fields.is_empty() { query_args.push(("exclude_fields".to_string(), exclude_fields.join(" "))); }
if !fields.is_empty() { query_args.push(("fields".to_string(), fields.join(" "))); }
if !is_read.to_string().is_empty() {  query_args.push(("is_read".to_string(), is_read.to_string())); }
if let Some(date) = since_timestamp { query_args.push(("since_timestamp".to_string(), date.to_rfc3339())); }
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
...
&self,
fields: &[String], exclude_fields: &[String], conversation_id: &str, message_id: &str,
...
let url =
format!("/conversations/{}/messages/{}?{}",
crate::progenitor_support::encode_path(&conversation_id.to_string()),crate::progenitor_support::encode_path(&message_id.to_string()),query_);
