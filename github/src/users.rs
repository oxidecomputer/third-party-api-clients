...
let url =
"/user/email/visibility".to_string();
self.client.patch(&url, Some(reqwest::Body::from(serde_json::to_vec(body).unwrap()))).await
}
...
&self,
username: &str, per_page: i64, page: i64,
) -> Result<Vec<crate::types::SimpleUser>> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if page > 0 { query_args.push(format!("page={}", page)); }
...
&self,
username: &str, target_user: &str,
) -> Result<()> {
let url =
format!("/users/{}/following/{}",
crate::progenitor_support::encode_path(&username.to_string()),crate::progenitor_support::encode_path(&target_user.to_string()),);
...
&self,
username: &str, subject_type: crate::types::SubjectType, subject_id: &str,
) -> Result<crate::types::Hovercard> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !subject_id.is_empty() { query_args.push(format!("subject_id={}", subject_id)); }
