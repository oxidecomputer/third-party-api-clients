...
&self,
owner: &str, repo: &str, tool_name: &str, tool_guid: &str, page: i64, per_page: i64, ref_: &str, state: crate::types::CodeScanningAlertState,
...
if !tool_guid.is_empty() { query_args.push(format!("tool_guid={}", tool_guid)); }
if !tool_name.is_empty() { query_args.push(format!("tool_name={}", tool_name)); }
...
let url =
format!("/repos/{}/{}/code-scanning/alerts?{}",
...
&self,
owner: &str, repo: &str, tool_name: &str, tool_guid: &str, ref_: &str, state: crate::types::CodeScanningAlertState,
) -> Result<Vec<crate::types::CodeScanningAlertItems>> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !ref_.is_empty() { query_args.push(format!("ref={}", ref_)); }
...
&self,
owner: &str, repo: &str, alert_number: i64, page: i64, per_page: i64, ref_: &str,
...
let url =
format!("/repos/{}/{}/code-scanning/alerts/{}/instances?{}",
crate::progenitor_support::encode_path(&owner.to_string()),crate::progenitor_support::encode_path(&repo.to_string()),crate::progenitor_support::encode_path(&alert_number.to_string()),query);
...
&self,
owner: &str, repo: &str, alert_number: i64, ref_: &str,
) -> Result<Vec<crate::types::CodeScanningAlertInstance>> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !ref_.is_empty() { query_args.push(format!("ref={}", ref_)); }
...
&self,
owner: &str, repo: &str, tool_name: &str, tool_guid: &str, page: i64, per_page: i64, ref_: &str, sarif_id: &str,
...
if per_page > 0 { query_args.push(format!("per_page={}", per_page)); }
if !ref_.is_empty() { query_args.push(format!("ref={}", ref_)); }
if !sarif_id.is_empty() { query_args.push(format!("sarif_id={}", sarif_id)); }
if !tool_guid.is_empty() { query_args.push(format!("tool_guid={}", tool_guid)); }
if !tool_name.is_empty() { query_args.push(format!("tool_name={}", tool_name)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
...
let url =
format!("/repos/{}/{}/code-scanning/analyses?{}",
crate::progenitor_support::encode_path(&owner.to_string()),crate::progenitor_support::encode_path(&repo.to_string()),query);
...
&self,
owner: &str, repo: &str, tool_name: &str, tool_guid: &str, ref_: &str, sarif_id: &str,
) -> Result<Vec<crate::types::CodeScanningAnalysis>> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !ref_.is_empty() { query_args.push(format!("ref={}", ref_)); }
...
&self,
owner: &str, repo: &str, analysis_id: i64,
) -> Result<crate::types::CodeScanningAnalysis> {
let url =
format!("/repos/{}/{}/code-scanning/analyses/{}",
crate::progenitor_support::encode_path(&owner.to_string()),crate::progenitor_support::encode_path(&repo.to_string()),crate::progenitor_support::encode_path(&analysis_id.to_string()),);
...
&self,
owner: &str, repo: &str, analysis_id: i64, confirm_delete: &str,
) -> Result<crate::types::AnalysisDeletion> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !confirm_delete.is_empty() { query_args.push(format!("confirm_delete={}", confirm_delete)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
                    }
...
let url =
format!("/repos/{}/{}/code-scanning/analyses/{}?{}",
crate::progenitor_support::encode_path(&owner.to_string()),crate::progenitor_support::encode_path(&repo.to_string()),crate::progenitor_support::encode_path(&analysis_id.to_string()),query);
...
let url =
format!("/repos/{}/{}/code-scanning/sarifs",
...
&self,
owner: &str, repo: &str, sarif_id: &str,
) -> Result<crate::types::CodeScanningSarifsStatus> {
let url =
format!("/repos/{}/{}/code-scanning/sarifs/{}",
crate::progenitor_support::encode_path(&owner.to_string()),crate::progenitor_support::encode_path(&repo.to_string()),crate::progenitor_support::encode_path(&sarif_id.to_string()),);
