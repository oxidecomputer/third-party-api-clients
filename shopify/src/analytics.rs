...
&self,
ids: &str, limit: &str, since_id: &str, updated_at_min: &str, updated_at_max: &str, fields: &str,
) -> Result<()> {
...
let url =
format!("/admin/api/2020-01/reports.json?{}",
query_);
...
&self,
name: &str, shopify_ql: &str,
body: &serde_json::Value
) -> Result<()> {
let mut query_args: Vec<(String, String)> = Default::default();
if !name.is_empty() { query_args.push(("name".to_string(), name.to_string())); }
if !shopify_ql.is_empty() { query_args.push(("shopify_ql".to_string(), shopify_ql.to_string())); }
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/admin/api/2020-01/reports.json?{}",
query_);
...
let url =
format!("/admin/api/2020-01/reports/{}/json?{}",
crate::progenitor_support::encode_path(&report_id.to_string()),query_);
...
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/admin/api/2020-04/reports.json?{}",
query_);
...
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/admin/api/2020-07/reports.json?{}",
query_);
...
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/admin/api/2020-10/reports.json?{}",
query_);
...
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/admin/api/2021-01/reports.json?{}",
query_);
...
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/admin/api/unstable/reports.json?{}",
query_);
