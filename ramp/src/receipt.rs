...
&self,
from_date: Option<chrono::DateTime<chrono::Utc>>, to_date: Option<chrono::DateTime<chrono::Utc>>, created_after: Option<chrono::DateTime<chrono::Utc>>, created_before: Option<chrono::DateTime<chrono::Utc>>, start: &str, page_size: f64,
) -> Result<crate::types::GetReceiptsResponse> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if let Some(date) = created_after { query_args.push(format!("created_after={}", &date.to_rfc3339())); }
if let Some(date) = created_before { query_args.push(format!("created_before={}", &date.to_rfc3339())); }
...
&self,
) -> Result<crate::types::GetReceiptsResponseData> {
let url =
"/receipts/<id>".to_string();
self.client.get(&url, None).await
}
