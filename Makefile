...
github: target/debug/generator $(GITHUB_SPEC)
	./target/debug/generator -i $(GITHUB_SPEC) -v 0.1.26 \
		-o github \
		-n octorust \
		--proper-name GitHub \
...
google-admin: target/debug/generator $(GOOGLE_ADMIN_SPEC)
	./target/debug/generator -i $(GOOGLE_ADMIN_SPEC) -v 0.2.2 \
		-o google/admin \
		-n gsuite-api \
...
google-calendar: target/debug/generator $(GOOGLE_CALENDAR_SPEC)
	./target/debug/generator -i $(GOOGLE_CALENDAR_SPEC) -v 0.1.2 \
		-o google/calendar \
		-n google-calendar \
...
google-drive: target/debug/generator $(GOOGLE_DRIVE_SPEC)
	./target/debug/generator -i $(GOOGLE_DRIVE_SPEC) -v 0.2.2 \
		-o google/drive \
		-n google-drive \
...
google-groups-settings: target/debug/generator $(GOOGLE_GROUPS_SETTINGS_SPEC)
	./target/debug/generator -i $(GOOGLE_GROUPS_SETTINGS_SPEC) -v 0.1.2 \
...
google-sheets: target/debug/generator $(GOOGLE_SHEETS_SPEC)
	./target/debug/generator -i $(GOOGLE_SHEETS_SPEC) -v 0.2.1 \
		-o google/sheets \
		-n sheets \
...
ramp: target/debug/generator $(RAMP_SPEC)
	./target/debug/generator -i $(RAMP_SPEC) -v 0.2.4 \
		-o ramp \
		-n ramp-api \
		--proper-name Ramp \
