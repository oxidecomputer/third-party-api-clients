...
&self,
parent_list_ids: &str, no_parent_list_id: bool,
) -> Result<crate::types::AllSegmentsResponse> {
let mut query_args: Vec<(String, String)> = Default::default();
if no_parent_list_id { query_args.push(("no_parent_list_id".to_string(), no_parent_list_id.to_string())); }
if !parent_list_ids.is_empty() { query_args.push(("parent_list_ids".to_string(), parent_list_ids.to_string())); }
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/marketing/segments/2.0?{}",
query_);
...
let url =
"/marketing/segments/2.0".to_string();
...
&self,
segment_id: &str, contacts_sample: bool,
) -> Result<crate::types::SegmentResponse> {
let mut query_args: Vec<(String, String)> = Default::default();
if contacts_sample { query_args.push(("contacts_sample".to_string(), contacts_sample.to_string())); }
let query_ = serde_urlencoded::to_string(&query_args).unwrap();
let url =
format!("/marketing/segments/2.0/{}?{}",
crate::progenitor_support::encode_path(&segment_id.to_string()),query_);
...
* **The Segmentation V2 API is currently in private beta. If you'd like to be added to the beta, please fill out this [form](https://docs.google.com/forms/d/e/1FAIpQLSd5zwC9dRk8lAp1oTWjdGc-aSY69flW_7wnutvKBhpUluSnfQ/viewform)**
*/
pub async fn delete_segments_segment(
&self,
segment_id: &str,
) -> Result<()> {
let url =
