...
&self,
xgafv: crate::types::Xgafv, access_token: &str, alt: crate::types::Alt, callback: &str, fields: &str, key: &str, oauth_token: &str, pretty_print: bool, quota_user: &str, upload_protocol: &str, upload_type: &str, group_key: &str, member_key: &str,
...
let url =
format!("/admin/directory/v1/groups/{}/hasMember/{}?{}",
crate::progenitor_support::encode_path(&group_key.to_string()),crate::progenitor_support::encode_path(&member_key.to_string()),query);
...
&self,
xgafv: crate::types::Xgafv, access_token: &str, alt: crate::types::Alt, callback: &str, fields: &str, key: &str, oauth_token: &str, pretty_print: bool, quota_user: &str, upload_protocol: &str, upload_type: &str, group_key: &str, include_derived_membership: bool, max_results: i64, page_token: &str, roles: &str,
) -> Result<crate::types::Members> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !access_token.is_empty() { query_args.push(format!("access_token={}", access_token)); }
query_args.push(format!("alt={}", alt));
if !callback.is_empty() { query_args.push(format!("callback={}", callback)); }
if !fields.is_empty() { query_args.push(format!("fields={}", fields)); }
if include_derived_membership { query_args.push(format!("include_derived_membership={}", include_derived_membership)); }
if !key.is_empty() { query_args.push(format!("key={}", key)); }
...
if pretty_print { query_args.push(format!("pretty_print={}", pretty_print)); }
if !quota_user.is_empty() { query_args.push(format!("quota_user={}", quota_user)); }
if !roles.is_empty() { query_args.push(format!("roles={}", roles)); }
if !upload_protocol.is_empty() { query_args.push(format!("upload_protocol={}", upload_protocol)); }
if !upload_type.is_empty() { query_args.push(format!("upload_type={}", upload_type)); }
query_args.push(format!("xgafv={}", xgafv));
for (i, n) in query_args.iter().enumerate() {
...
let url =
format!("/admin/directory/v1/groups/{}/members?{}",
