...
&self,
alt: crate::types::Alt, fields: &str, key: &str, oauth_token: &str, pretty_print: bool, quota_user: &str, user_ip: &str, calendar_id: &str, always_include_email: bool, i_cal_uid: &str, max_attendees: i64, max_results: i64, order_by: crate::types::OrderBy, page_token: &str, private_extended_property: &[String], q: &str, shared_extended_property: &[String], show_deleted: bool, show_hidden_invitations: bool, single_events: bool, sync_token: &str, time_max: &str, time_min: &str, time_zone: &str, updated_min: &str,
) -> Result<crate::types::Events> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
query_args.push(format!("alt={}", alt));
if always_include_email { query_args.push(format!("always_include_email={}", always_include_email)); }
if !fields.is_empty() { query_args.push(format!("fields={}", fields)); }
if !i_cal_uid.is_empty() { query_args.push(format!("i_cal_uid={}", i_cal_uid)); }
if !key.is_empty() { query_args.push(format!("key={}", key)); }
if max_attendees > 0 { query_args.push(format!("max_attendees={}", max_attendees)); }
...
if !private_extended_property.is_empty() { query_args.push(format!("private_extended_property={}", private_extended_property.join(" "))); }
if !q.is_empty() { query_args.push(format!("q={}", q)); }
if !quota_user.is_empty() { query_args.push(format!("quota_user={}", quota_user)); }
if !shared_extended_property.is_empty() { query_args.push(format!("shared_extended_property={}", shared_extended_property.join(" "))); }
if show_deleted { query_args.push(format!("show_deleted={}", show_deleted)); }
if show_hidden_invitations { query_args.push(format!("show_hidden_invitations={}", show_hidden_invitations)); }
if single_events { query_args.push(format!("single_events={}", single_events)); }
if !sync_token.is_empty() { query_args.push(format!("sync_token={}", sync_token)); }
if !time_max.is_empty() { query_args.push(format!("time_max={}", time_max)); }
if !time_min.is_empty() { query_args.push(format!("time_min={}", time_min)); }
if !time_zone.is_empty() { query_args.push(format!("time_zone={}", time_zone)); }
if !updated_min.is_empty() { query_args.push(format!("updated_min={}", updated_min)); }
...
&self,
alt: crate::types::Alt, fields: &str, key: &str, oauth_token: &str, pretty_print: bool, quota_user: &str, user_ip: &str, calendar_id: &str, conference_data_version: u64, max_attendees: i64, send_notifications: bool, send_updates: crate::types::SendUpdates, supports_attachments: bool,
body: &crate::types::EventData
) -> Result<crate::types::EventData> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
query_args.push(format!("alt={}", alt));
query_args.push(format!("conference_data_version={}", conference_data_version));
if !fields.is_empty() { query_args.push(format!("fields={}", fields)); }
if !key.is_empty() { query_args.push(format!("key={}", key)); }
if max_attendees > 0 { query_args.push(format!("max_attendees={}", max_attendees)); }
if !oauth_token.is_empty() { query_args.push(format!("oauth_token={}", oauth_token)); }
...
if supports_attachments { query_args.push(format!("supports_attachments={}", supports_attachments)); }
if !user_ip.is_empty() { query_args.push(format!("user_ip={}", user_ip)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
...
&self,
alt: crate::types::Alt, fields: &str, key: &str, oauth_token: &str, pretty_print: bool, quota_user: &str, user_ip: &str, calendar_id: &str, conference_data_version: u64, supports_attachments: bool,
body: &crate::types::EventData
) -> Result<crate::types::EventData> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
...
let url =
format!("/calendars/{}/events/import?{}",
crate::progenitor_support::encode_path(&calendar_id.to_string()),query);
...
&self,
alt: crate::types::Alt, fields: &str, key: &str, oauth_token: &str, pretty_print: bool, quota_user: &str, user_ip: &str, calendar_id: &str, text: &str, send_notifications: bool, send_updates: crate::types::SendUpdates,
) -> Result<crate::types::EventData> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
query_args.push(format!("alt={}", alt));
...
if send_notifications { query_args.push(format!("send_notifications={}", send_notifications)); }
query_args.push(format!("send_updates={}", send_updates));
if !text.is_empty() { query_args.push(format!("text={}", text)); }
if !user_ip.is_empty() { query_args.push(format!("user_ip={}", user_ip)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
...
let url =
format!("/calendars/{}/events/quickAdd?{}",
crate::progenitor_support::encode_path(&calendar_id.to_string()),query);
...
&self,
alt: crate::types::Alt, fields: &str, key: &str, oauth_token: &str, pretty_print: bool, quota_user: &str, user_ip: &str, calendar_id: &str, event_id: &str, always_include_email: bool, max_attendees: i64, time_zone: &str,
) -> Result<crate::types::EventData> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
query_args.push(format!("alt={}", alt));
...
let url =
format!("/calendars/{}/events/{}?{}",
crate::progenitor_support::encode_path(&calendar_id.to_string()),crate::progenitor_support::encode_path(&event_id.to_string()),query);
...
&self,
alt: crate::types::Alt, fields: &str, key: &str, oauth_token: &str, pretty_print: bool, quota_user: &str, user_ip: &str, calendar_id: &str, event_id: &str, always_include_email: bool, conference_data_version: u64, max_attendees: i64, send_notifications: bool, send_updates: crate::types::SendUpdates, supports_attachments: bool,
body: &crate::types::EventData
) -> Result<crate::types::EventData> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
...
&self,
alt: crate::types::Alt, fields: &str, key: &str, oauth_token: &str, pretty_print: bool, quota_user: &str, user_ip: &str, calendar_id: &str, event_id: &str, send_notifications: bool, send_updates: crate::types::SendUpdates,
) -> Result<()> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
query_args.push(format!("alt={}", alt));
...
&self,
alt: crate::types::Alt, fields: &str, key: &str, oauth_token: &str, pretty_print: bool, quota_user: &str, user_ip: &str, calendar_id: &str, event_id: &str, always_include_email: bool, max_attendees: i64, max_results: i64, original_start: &str, page_token: &str, show_deleted: bool, time_max: &str, time_min: &str, time_zone: &str,
) -> Result<crate::types::Events> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
query_args.push(format!("alt={}", alt));
...
if max_results > 0 { query_args.push(format!("max_results={}", max_results)); }
if !oauth_token.is_empty() { query_args.push(format!("oauth_token={}", oauth_token)); }
if !original_start.is_empty() { query_args.push(format!("original_start={}", original_start)); }
if !page_token.is_empty() { query_args.push(format!("page_token={}", page_token)); }
if pretty_print { query_args.push(format!("pretty_print={}", pretty_print)); }
if !quota_user.is_empty() { query_args.push(format!("quota_user={}", quota_user)); }
if show_deleted { query_args.push(format!("show_deleted={}", show_deleted)); }
...
let url =
format!("/calendars/{}/events/{}/instances?{}",
crate::progenitor_support::encode_path(&calendar_id.to_string()),crate::progenitor_support::encode_path(&event_id.to_string()),query);
...
&self,
alt: crate::types::Alt, fields: &str, key: &str, oauth_token: &str, pretty_print: bool, quota_user: &str, user_ip: &str, calendar_id: &str, event_id: &str, destination: &str, send_notifications: bool, send_updates: crate::types::SendUpdates,
) -> Result<crate::types::EventData> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
query_args.push(format!("alt={}", alt));
if !destination.is_empty() { query_args.push(format!("destination={}", destination)); }
if !fields.is_empty() { query_args.push(format!("fields={}", fields)); }
if !key.is_empty() { query_args.push(format!("key={}", key)); }
if !oauth_token.is_empty() { query_args.push(format!("oauth_token={}", oauth_token)); }
if pretty_print { query_args.push(format!("pretty_print={}", pretty_print)); }
