...
&self,
search_key: &str, query_presence_status: &str, page_size: i64, next_page_token: &str,
...
if !query_presence_status.is_empty() { query_args.push(format!("query_presence_status={}", query_presence_status)); }
if !search_key.is_empty() { query_args.push(format!("search_key={}", search_key)); }
...
&self,
type_: &str, page_size: i64, next_page_token: &str,
...
pub async fn get_user_contact(
&self,
contact_id: &str, contact_id: &str, query_presence_status: bool,
) -> Result<crate::types::GetUserContactResponse> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if query_presence_status { query_args.push(format!("query_presence_status={}", query_presence_status)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
                    }
...
let url =
format!("/chat/users/me/contacts/{}?{}",
crate::progenitor_support::encode_path(&contact_id.to_string()),query);
