...
&self,
user_id: &str, to_contact: &str, to_channel: &str, date: chrono::NaiveDate, page_size: i64, next_page_token: &str, include_deleted_and_edited_message: &str,
) -> Result<crate::types::GetChatMessagesResponse> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
query_args.push(format!("date={}", date));
if !include_deleted_and_edited_message.is_empty() { query_args.push(format!("include_deleted_and_edited_message={}", include_deleted_and_edited_message)); }
...
if !to_channel.is_empty() { query_args.push(format!("to_channel={}", to_channel)); }
if !to_contact.is_empty() { query_args.push(format!("to_contact={}", to_contact)); }
...
let url =
format!("/chat/users/{}/messages/{}/status",
crate::progenitor_support::encode_path(&user_id.to_string()),crate::progenitor_support::encode_path(&message_id.to_string()),);
...
let url =
format!("/chat/users/{}/messages/{}/emoji_reactions",
crate::progenitor_support::encode_path(&user_id.to_string()),crate::progenitor_support::encode_path(&message_id.to_string()),);
...
&self,
user_id: &str, message_id: &str, message_id: &str, to_contact: &str, to_channel: &str,
...
let url =
format!("/chat/users/{}/messages/{}?{}",
crate::progenitor_support::encode_path(&user_id.to_string()),crate::progenitor_support::encode_path(&message_id.to_string()),query);
...
&self,
user_id: &str, message_id: &str, message_id: &str,
