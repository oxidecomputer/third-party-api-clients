...
&self,
next_page_token: &str, type_: crate::types::ListAccountNumbersType, extension_type: crate::types::ExtensionType, page_size: i64, number_type: crate::types::Type, pending_numbers: bool, site_id: &str,
...
if pending_numbers { query_args.push(format!("pending_numbers={}", pending_numbers)); }
if !site_id.is_empty() { query_args.push(format!("site_id={}", site_id)); }
...
&self,
account_id: &str,
) -> Result<crate::types::SettingResponse> {
let url =
"/phone/settings".to_string();
self.client.get(&url, None).await
}
...
&self,
user_id: &str,
) -> Result<crate::types::UserSettingsResponse> {
let url =
format!("/phone/users/{}/settings",
crate::progenitor_support::encode_path(&user_id.to_string()),);
...
&self,
page_size: i64, next_page_token: &str, site_id: &str,
...
let url =
format!("/phone/setting_templates?{}",
query);
...
let url =
"/phone/setting_templates".to_string();
self.client.post(&url, Some(reqwest::Body::from(serde_json::to_vec(body).unwrap()))).await
}
...
let url =
"/phone/batch_locations".to_string();
self.client.post(&url, Some(reqwest::Body::from(serde_json::to_vec(body).unwrap()))).await
}
...
&self,
next_page_token: &str, page_size: i64,
...
&self,
template_id: &str, template_id: &str, custom_query_fields: &str,
) -> Result<crate::types::GetSettingTemplateResponse> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !custom_query_fields.is_empty() { query_args.push(format!("custom_query_fields={}", custom_query_fields)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
                    }
...
let url =
format!("/phone/setting_templates/{}?{}",
crate::progenitor_support::encode_path(&template_id.to_string()),query);
...
&self,
user_id: &str, page_size: i64, from: chrono::NaiveDate, to: chrono::NaiveDate, type_: crate::types::UserCallLogsType, next_page_token: &str, phone_number: &str, time_type: crate::types::TimeType,
...
if !next_page_token.is_empty() { query_args.push(format!("next_page_token={}", next_page_token)); }
if page_size > 0 { query_args.push(format!("page_size={}", page_size)); }
if !phone_number.is_empty() { query_args.push(format!("phone_number={}", phone_number)); }
...
&self,
user_id: &str, page_size: i64, next_page_token: &str, from: chrono::NaiveDate, to: chrono::NaiveDate,
...
&self,
user_id: &str, page_size: i64, status: crate::types::UserVoiceMailsStatus, next_page_token: &str, from: chrono::NaiveDate, to: chrono::NaiveDate,
...
let url =
format!("/phone/users/{}/voice_mails?{}",
crate::progenitor_support::encode_path(&user_id.to_string()),query);
...
&self,
user_id: &str, setting_type: &str,
body: &crate::types::AddUserSettingRequest
) -> Result<crate::types::AddUserSettingResponse> {
let url =
format!("/phone/users/{}/settings/{}",
crate::progenitor_support::encode_path(&user_id.to_string()),crate::progenitor_support::encode_path(&setting_type.to_string()),);
...
&self,
user_id: &str, setting_type: &str, shared_id: &str,
) -> Result<()> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !shared_id.is_empty() { query_args.push(format!("shared_id={}", shared_id)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
                    }
...
let url =
format!("/phone/users/{}/settings/{}?{}",
crate::progenitor_support::encode_path(&user_id.to_string()),crate::progenitor_support::encode_path(&setting_type.to_string()),query);
...
&self,
page_size: i64, from: &str, to: &str, type_: &str, next_page_token: &str, path: &str, time_type: crate::types::TimeType, site_id: &str,
) -> Result<crate::types::AccountCallLogsResponseData> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !from.is_empty() { query_args.push(format!("from={}", from)); }
if !next_page_token.is_empty() { query_args.push(format!("next_page_token={}", next_page_token)); }
if page_size > 0 { query_args.push(format!("page_size={}", page_size)); }
if !path.is_empty() { query_args.push(format!("path={}", path)); }
if !site_id.is_empty() { query_args.push(format!("site_id={}", site_id)); }
query_args.push(format!("time_type={}", time_type));
if !to.is_empty() { query_args.push(format!("to={}", to)); }
...
let url =
format!("/phone/users/{}/phone_numbers",
crate::progenitor_support::encode_path(&user_id.to_string()),);
...
&self,
user_id: &str, phone_number_id: &str, user_id: &str, phone_number_id: &str,
) -> Result<()> {
let url =
format!("/phone/users/{}/phone_numbers/{}",
crate::progenitor_support::encode_path(&user_id.to_string()),crate::progenitor_support::encode_path(&phone_number_id.to_string()),);
...
let url =
format!("/phone/users/{}/calling_plans",
crate::progenitor_support::encode_path(&user_id.to_string()),);
...
&self,
user_id: &str, type_: &str, type_: &str,
) -> Result<()> {
let url =
format!("/phone/users/{}/calling_plans/{}",
crate::progenitor_support::encode_path(&user_id.to_string()),crate::progenitor_support::encode_path(&type_.to_string()),);
...
&self,
page_size: i64, next_page_token: &str, from: &str, to: &str, owner_type: &str, recording_type: &str, site_id: &str, query_date_type: crate::types::QueryDateType,
) -> Result<crate::types::GetRecordingsResponseData> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !from.is_empty() { query_args.push(format!("from={}", from)); }
if !next_page_token.is_empty() { query_args.push(format!("next_page_token={}", next_page_token)); }
if !owner_type.is_empty() { query_args.push(format!("owner_type={}", owner_type)); }
if page_size > 0 { query_args.push(format!("page_size={}", page_size)); }
query_args.push(format!("query_date_type={}", query_date_type));
if !recording_type.is_empty() { query_args.push(format!("recording_type={}", recording_type)); }
if !site_id.is_empty() { query_args.push(format!("site_id={}", site_id)); }
if !to.is_empty() { query_args.push(format!("to={}", to)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
...
let url =
format!("/accounts/{}/phone/sip_trunk/trunks",
crate::progenitor_support::encode_path(&account_id.to_string()),);
...
&self,
sip_trunk_id: &str, account_id: &str,
body: &crate::types::UpdateSipTrunkRequest
) -> Result<()> {
let url =
format!("/accounts/{}/phone/sip_trunk/trunks/{}",
crate::progenitor_support::encode_path(&account_id.to_string()),crate::progenitor_support::encode_path(&sip_trunk_id.to_string()),);
...
let url =
format!("/phone/external_contacts?{}",
query);
...
let url =
"/phone/external_contacts".to_string();
self.client.post(&url, Some(reqwest::Body::from(serde_json::to_vec(body).unwrap()))).await
}
...
&self,
external_contact_id: &str, external_contact_id: &str,
...
let url =
"/phone/company_number".to_string();
...
&self,
) -> Result<crate::types::ListCallingPlansResponseData> {
let url =
"/phone/calling_plans".to_string();
self.client.get(&url, None).await
}
...
&self,
call_log_id: &str, call_log_id: &str,
...
&self,
user_id: &str, call_log_id: &str, user_id: &str, call_log_id: &str,
) -> Result<()> {
let url =
format!("/phone/users/{}/call_logs/{}",
crate::progenitor_support::encode_path(&user_id.to_string()),crate::progenitor_support::encode_path(&call_log_id.to_string()),);
...
&self,
voicemail_id: &str, voicemail_id: &str,
) -> Result<()> {
let url =
format!("/phone/voice_mails/{}",
