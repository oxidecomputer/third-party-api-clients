...
&self,
user_id: &str, page_size: i64, next_page_token: &str, mc: &str, trash: bool, from: chrono::NaiveDate, to: chrono::NaiveDate, trash_type: &str,
) -> Result<crate::types::RecordingMeetingList> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
query_args.push(format!("from={}", from));
if !mc.is_empty() { query_args.push(format!("mc={}", mc)); }
...
if trash { query_args.push(format!("trash={}", trash)); }
if !trash_type.is_empty() { query_args.push(format!("trash_type={}", trash_type)); }
...
&self,
meeting_id: &str, include_fields: &str, ttl: u64,
) -> Result<crate::types::RecordingMeeting> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !include_fields.is_empty() { query_args.push(format!("include_fields={}", include_fields)); }
...
&self,
meeting_id: &str, action: crate::types::RecordingDeleteAction,
) -> Result<()> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
...
&self,
meeting_id: &str, recording_id: &str, action: crate::types::RecordingDeleteAction,
) -> Result<()> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
query_args.push(format!("action={}", action));
...
let url =
format!("/meetings/{}/recordings/{}?{}",
crate::progenitor_support::encode_path(&meeting_id.to_string()),crate::progenitor_support::encode_path(&recording_id.to_string()),query);
...
let url =
format!("/meetings/{}/recordings/status",
...
let url =
format!("/meetings/{}/recordings/{}/status",
crate::progenitor_support::encode_path(&meeting_id.to_string()),crate::progenitor_support::encode_path(&recording_id.to_string()),);
...
let url =
format!("/meetings/{}/recordings/settings",
crate::progenitor_support::encode_path(&meeting_id.to_string()),);
...
&self,
meeting_id: i64, status: crate::types::RegistrantStatusData, page_size: i64, page_number: i64, next_page_token: &str,
...
let url =
format!("/meetings/{}/recordings/registrants?{}",
crate::progenitor_support::encode_path(&meeting_id.to_string()),query);
...
let url =
format!("/meetings/{}/recordings/registrants/status",
crate::progenitor_support::encode_path(&meeting_id.to_string()),);
...
let url =
format!("/meetings/{}/recordings/registrants/questions",
crate::progenitor_support::encode_path(&meeting_id.to_string()),);
...
&self,
account_id: &str, page_size: i64, next_page_token: &str, from: Option<chrono::DateTime<chrono::Utc>>, to: Option<chrono::DateTime<chrono::Utc>>, account_id: &str,
) -> Result<crate::types::GetAccountCloudRecordingResponse> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if let Some(date) = from { query_args.push(format!("from={}", &date.to_rfc3339())); }
if !next_page_token.is_empty() { query_args.push(format!("next_page_token={}", next_page_token)); }
if page_size > 0 { query_args.push(format!("page_size={}", page_size)); }
if let Some(date) = to { query_args.push(format!("to={}", &date.to_rfc3339())); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
                    }
