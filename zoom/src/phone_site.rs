...
&self,
site_id: &str, site_id: &str, transfer_site_id: &str,
) -> Result<()> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !transfer_site_id.is_empty() { query_args.push(format!("transfer_site_id={}", transfer_site_id)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
                    }
...
let url =
format!("/phone/sites/{}?{}",
crate::progenitor_support::encode_path(&site_id.to_string()),query);
