...
&self,
role_id: &str, page_count: &str, page_number: i64, next_page_token: &str, page_size: i64,
...
if !page_count.is_empty() { query_args.push(format!("page_count={}", page_count)); }
...
let url =
format!("/roles/{}/members?{}",
crate::progenitor_support::encode_path(&role_id.to_string()),query);
...
&self,
role_id: &str, member_id: &str,
) -> Result<()> {
let url =
format!("/roles/{}/members/{}",
crate::progenitor_support::encode_path(&role_id.to_string()),crate::progenitor_support::encode_path(&member_id.to_string()),);
