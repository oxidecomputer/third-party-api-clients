...
*/
pub async fn list_zr_location(
&self,
parent_location_id: &str, type_: &str, page_size: i64, next_page_token: &str,
) -> Result<crate::types::ListZrLocationsResponseData> {
...
if !parent_location_id.is_empty() { query_args.push(format!("parent_location_id={}", parent_location_id)); }
...
&self,
location_id: &str, setting_type: &str, location_id: &str,
...
let url =
format!("/rooms/locations/{}/settings?{}",
crate::progenitor_support::encode_path(&location_id.to_string()),query);
...
let url =
"/rooms/locations/structure".to_string();
self.client.get(&url, None).await
}
...
let url =
format!("/rooms/locations/{}/location",
crate::progenitor_support::encode_path(&location_id.to_string()),);
