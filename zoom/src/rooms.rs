...
&self,
status: crate::types::Status, type_: crate::types::ListZoomRoomsType, unassigned_rooms: bool, page_size: i64, next_page_token: &str, location_id: &str,
...
if unassigned_rooms { query_args.push(format!("unassigned_rooms={}", unassigned_rooms)); }
...
&self,
room_id: &str, setting_type: &str, room_id: &str,
) -> Result<crate::types::Domains> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !setting_type.is_empty() { query_args.push(format!("setting_type={}", setting_type)); }
for (i, n) in query_args.iter().enumerate() {
                    if i > 0 {
                        query.push('&');
                    }
...
let url =
format!("/rooms/{}/settings?{}",
crate::progenitor_support::encode_path(&room_id.to_string()),query);
...
&self,
room_id: &str, room_id: &str, setting_type: &str,
) -> Result<()> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !setting_type.is_empty() { query_args.push(format!("setting_type={}", setting_type)); }
...
&self,
type_: &str, folder_id: &str, page_size: i64, next_page_token: &str,
) -> Result<crate::types::ListDigitalSignageContentResponse> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !folder_id.is_empty() { query_args.push(format!("folder_id={}", folder_id)); }
if !next_page_token.is_empty() { query_args.push(format!("next_page_token={}", next_page_token)); }
if page_size > 0 { query_args.push(format!("page_size={}", page_size)); }
...
* 
*/
pub async fn manage_e91_1signage(
&self,
) -> Result<crate::types::ManageE911SignageResponse> {
let url =
"/rooms/events".to_string();
self.client.patch(&url, None).await
}
