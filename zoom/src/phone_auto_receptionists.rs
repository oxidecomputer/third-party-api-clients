...
&self,
auto_receptionist_id: &str, auto_receptionist_id: &str,
body: &crate::types::UpdateAutoReceptionistRequest
) -> Result<()> {
let url =
format!("/phone/auto_receptionists/{}",
...
let url =
format!("/phone/auto_receptionists/{}/phone_numbers",
crate::progenitor_support::encode_path(&auto_receptionist_id.to_string()),);
...
&self,
auto_receptionist_id: &str, phone_number_id: &str, auto_receptionist_id: &str, phone_number_id: &str,
) -> Result<()> {
let url =
format!("/phone/auto_receptionists/{}/phone_numbers/{}",
crate::progenitor_support::encode_path(&auto_receptionist_id.to_string()),crate::progenitor_support::encode_path(&phone_number_id.to_string()),);
...
let url =
"/phone/auto_receptionists".to_string();
self.client.post(&url, Some(reqwest::Body::from(serde_json::to_vec(body).unwrap()))).await
}
