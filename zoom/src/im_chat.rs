...
&self,
session_id: &str, from: chrono::NaiveDate, to: chrono::NaiveDate, page_size: i64, next_page_token: &str,
...
let url =
format!("/im/chat/sessions/{}?{}",
crate::progenitor_support::encode_path(&session_id.to_string()),query);
...
&self,
user_id: &str, chat_user: &str, channel: &str, date: &str, page_size: i64, next_page_token: &str,
) -> Result<crate::types::ListimmessagesResponse> {
let mut query = String::new();
let mut query_args: Vec<String> = Default::default();
if !channel.is_empty() { query_args.push(format!("channel={}", channel)); }
if !chat_user.is_empty() { query_args.push(format!("chat_user={}", chat_user)); }
...
let url =
format!("/im/users/me/chat/messages?{}",
query);
